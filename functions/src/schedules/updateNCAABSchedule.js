...
const {processNCAABScheduleFromJSON} =
 ...
 */
exports.updateNCAABSchedule = onCall(
    {
    ...
    async (request) => {
      ...
      try {
        console.log("NCAAB schedule upload triggered");
        ...
        const ncaabConfig = config.ncaab;

        if (!ncaabConfig) {
          throw new Error("NCAAB config not found in scheduleConfig");
        }

        // Process the JSON data
        const scheduleData = await processNCAABScheduleFromJSON(
            jsonData,
            ncaabConfig.team,
            ncaabConfig.season,
        );
        ...
        // Load mapping
        const mappingDocId = `ncaab-${ncaabConfig.team}-${ncaabConfig.season}`;
        ...
        console.error("NCABB upload failed:", error);
        throw error;
      }
    },
);
