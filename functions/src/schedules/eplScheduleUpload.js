...
const {processEPLScheduleFromJSON} =
 ...
 */
exports.updateEPLSchedule = onCall(
    {
    ...
    async (request) => {
      ...
      try {
        console.log("EPL schedule upload triggered");
        ...
        const eplConfig = config.epl;

        if (!eplConfig) {
          throw new Error("EPL config not found in scheduleConfig");
        }

        // Process the JSON data
        const scheduleData = await processEPLScheduleFromJSON(
            jsonData,
            eplConfig.team,
            eplConfig.season,
        );
        ...
        // Load mapping
        const mappingDocId = `epl-${eplConfig.team}-${eplConfig.season}`;
        ...
        console.error("EPL upload failed:", error);
        throw error;
      }
    },
);
