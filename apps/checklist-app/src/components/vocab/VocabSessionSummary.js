...
/**
 * Shown when a quiz session ends (queue exhausted in Once or Add Missed mode).
 *
 * Props:
 ...
 *   updatedItems    {object[]} - Items with this session's stats merged in
 */
const VocabSessionSummary = ({
  sessionResults,
  totalSessions,
  onPracticeAgain,
  updatedItems,
  ...
  const styles = StyleSheet.create({
    ...
    correctPill: { backgroundColor: theme.success },
    incorrectPill: { backgroundColor: theme.error },
    skippedPill: { backgroundColor: theme.border },
          ...
          return (
            <View key={item.id} style={styles.troubleItem}>
              <View style={styles.troubleRow}>
                <Text style={styles.troubleWord}>{item.word}</Text>
                <Text style={styles.ratioText}>
                  {allTimeCorrect}✓ / {allTimeIncorrect}✗ all-time
                </Text>
              </View>
              {!!item.definition && (
                <Text style={styles.troubleDefinition}>{item.definition}</Text>
              )}
            </View>
          );
        })
