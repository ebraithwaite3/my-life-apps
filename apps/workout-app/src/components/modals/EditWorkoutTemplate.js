      ...
      if (isTemplate) {
        ...
        const templateData = {
          ...
          exercises: selectedExercises.map((exerciseId, index) => ({
            exerciseId,
            order: index,
            setCount: setCounts[exerciseId] || 3,
          })),
        ...
        // Workouts - PRESERVE existing sets when editing
        const workoutData = {
          ...
          exercises: selectedExercises.map((exerciseId, index) => {
            ...
            // CRITICAL FIX: Find existing exercise data to preserve sets
            const existingExercise = workout?.exercises?.find(
              (ex) => ex.exerciseId === exerciseId
            );

            return {
              id: existingExercise?.id || `exercise-${Date.now()}-${index}`,
              exerciseId,
              order: index,
              sets: (() => {
                if (existingExercise?.sets) {
                  // PRESERVE existing sets, adjust count if needed
                  const existingSets = existingExercise.sets;

                  if (existingSets.length === setCount) {
                    // Same count - keep all existing sets
                    return existingSets;
                  } else if (existingSets.length < setCount) {
                    // Need MORE sets - keep existing, add new empty ones
                    const newSets = Array(setCount - existingSets.length)
                      .fill(null)
                      .map((_, i) => {
                        const set = {
                          id: `set-${Date.now()}-${index}-${
                            existingSets.length + i
                          }`,
                          completed: false,
                        };
                        if (exercise?.tracking?.includes("reps")) set.reps = 0;
                        if (exercise?.tracking?.includes("weight"))
                          set.weight = 0;
                        if (exercise?.tracking?.includes("distance"))
                          set.distance = 0;
                        if (exercise?.tracking?.includes("time")) set.time = 0;
                        return set;
                      });
                    return [...existingSets, ...newSets];
                  } else {
                    // Need FEWER sets - keep only first N sets
                    return existingSets.slice(0, setCount);
                  }
                } else {
                  // NEW exercise - create fresh empty sets
            ...
            <View style={styles.setCounter}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setSetCounts((c) => ({
                    ...c,
                    [exercise.id]: Math.max(1, setCount - 1),
                  }));
                }}
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setSetCounts((c) => ({
                    ...c,
                    [exercise.id]: Math.min(10, setCount + 1),
                  }));
                }}
                hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
              >
            ...
            <View style={styles.searchResults}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  return renderExerciseItem(item, isSelected, () =>
                    handleToggleInSearch(item.id)
                  );
                }}
              />
            </View>
              ...
              {selectedExercisesData.length > 0 && (
                <>
                  <View style={styles.resultsHeader}>
                    ...
                    <Text style={styles.resultsCount}>
                      {selectedExercisesData.length}{" "}
                      {selectedExercisesData.length === 1
                        ? "exercise"
                        : "exercises"}
                    </Text>
                  </View>
                  ...
                  <FlatList
                    ...
                    keyboardShouldPersistTaps="always"
                    renderItem={({ item }) => {
                      return renderExerciseItem(item, true, () =>
                        handleToggleInSearch(item.id)
                      );
                    }}
                  />
        ...
        >
          <View style={styles.section}>
            ...
            <TextInput
              ...
              placeholder={
                isTemplate ? "e.g., Push Day A" : "e.g., Morning Workout"
              }
              placeholderTextColor={theme.text.tertiary}
              value={name}
