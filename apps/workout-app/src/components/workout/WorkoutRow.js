...
const WorkoutRow = forwardRef(
  (
    ...
    const formatPreviousSet = () => {
      if (!previousSet) return "N/A";

      const parts = [];

      if (hasWeight && previousSet.weight) {
        parts.push(`${previousSet.weight} lbs`);
      }

      if (hasReps && previousSet.reps) {
        parts.push(`${previousSet.reps} reps`);
      }

      if (hasDistance && previousSet.distance) {
        parts.push(`${previousSet.distance} mi`);
      }

      if (hasTime && previousSet.time) {
        parts.push(formatTime(previousSet.time));
      }

      if (parts.length === 0) return "N/A";

      // Format based on combination
      if (hasWeight && hasReps) {
        return `${previousSet.weight} lbs × ${previousSet.reps}`;
      }

      if (hasDistance && hasTime) {
        return `${previousSet.distance} mi in ${formatTime(previousSet.time)}`;
      }
    ...
    // Check if current set is empty (all 0s)
    const isSetEmpty = () => {
      if (hasWeight && (set.weight || 0) > 0) return false;
      if (hasReps && (set.reps || 0) > 0) return false;
      if (hasDistance && (set.distance || 0) > 0) return false;
      if (hasTime && (set.time || 0) > 0) return false;
      return true;
    };

    // Determine if copy icon should show
    const showCopyIcon = previousSet && isSetEmpty() && !set.completed;

    // Handle copy previous set
    const handleCopyPrevious = () => {
      ...
      if (hasWeight && previousSet.weight) {
        updates.weight = previousSet.weight;
        setWeightInput(previousSet.weight.toString());
      }

      if (hasReps && previousSet.reps) {
        updates.reps = previousSet.reps;
        setRepsInput(previousSet.reps.toString());
      }

      if (hasDistance && previousSet.distance) {
        updates.distance = previousSet.distance;
        setDistanceInput(previousSet.distance.toString());
      }

      if (hasTime && previousSet.time) {
        updates.time = previousSet.time;
        setTimeInput(formatTime(previousSet.time));
      }
    ...
    return (
      <Swipeable renderRightActions={renderRightActions} overshootRight={false}>
        <View style={styles.container}>
          ...
          {/* Previous Set with Copy Icon */}
          <View style={styles.prevContainer}>
            <Text style={styles.prevText}>{formatPreviousSet()}</Text>
            {showCopyIcon && (
              <TouchableOpacity
          ...
          {hasWeight && (
            <TextInput
              ...
              onFocus={() => {
                ...
                onFocus?.("weight", weightRef.current);
              }}
          ...
          {hasDistance && (
            <TextInput
              ...
              onFocus={() => {
                ...
                onFocus?.("distance", distanceRef.current);
              }}
              keyboardType="decimal-pad"
              selectTextOnFocus
            />
