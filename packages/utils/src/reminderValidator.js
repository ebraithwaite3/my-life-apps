// Single source of truth for reminder validation.
// Validates against the canonical schema defined in
// functions/src/notifications/NOTIFICATIONS_SCHEMA.md.
//
// Dual-tolerant: accepts both the current flat schema
// (recurringIntervalDays, lastScheduledOccurrence, buttons, etc.) and the
// target nested schema (recurrence, scheduledAlertTime, actions, etc.) so
// validation can be enforced before the full migration is complete.
 ...
 *
 * Works with both the current flat schema and the target nested schema so
 * validation can be enforced before the full migration is complete. See
 * functions/src/notifications/NOTIFICATIONS_SCHEMA.md for field documentation.
 *
...
export function validateReminder(reminder) {
  ...
  if (!reminder || typeof reminder !== 'object' || Array.isArray(reminder)) {
    return { valid: false, errors: ['reminder must be a non-null object'], warnings: [] };
  }
  ...
  if (!DELIVERY_MODES.includes(reminder.deliveryMode)) {
    errors.push(
      `deliveryMode: must be one of ${DELIVERY_MODES.join(', ')} (got "${reminder.deliveryMode}")`,
    );
  }
  ...
  // Anchor field: scheduledAlertTime (new schema) or lastScheduledOccurrence (old schema).
  // Required for recurring reminders so retry/pending checks work correctly.
  const isOneTime =
  ...
  if (isValidISO(reminder.scheduledAlertTime) && isValidISO(reminder.scheduledTime)) {
    if (new Date(reminder.scheduledAlertTime) > new Date(reminder.scheduledTime)) {
      warnings.push(
  ...
  const hasOldRecurrence = !!(
    (Array.isArray(reminder.recurringSchedule) && reminder.recurringSchedule.length) ||
  ...
  if (!hasNewRecurrence && !hasOldRecurrence && !isOneTime) {
    errors.push(
      'recurrence: no recurrence type found — set recurrence.scheduleByDay, ' +
      'recurrence.everyNDays, recurrence.everyNMinutes, recurrence.oneTime, ' +
  ...
  if (hasNewRecurrence) {
    ...
    const activeTypes = ['scheduleByDay', 'everyNDays', 'everyNMinutes', 'oneTime']
      .filter((k) => r[k] !== undefined);

    if (activeTypes.length === 0) {
      errors.push(
        'recurrence: must contain exactly one of scheduleByDay, everyNDays, ' +
        'everyNMinutes, oneTime',
      );
    } else if (activeTypes.length > 1) {
      errors.push(
        `recurrence: multiple types set (${activeTypes.join(', ')}) — pick exactly one`,
      );
    } else {
        ...
        if (!Array.isArray(r.scheduleByDay) || r.scheduleByDay.length === 0) {
          ...
          r.scheduleByDay.forEach((entry, i) => {
            if (!VALID_DAYS.includes(entry.day)) {
              errors.push(
                `recurrence.scheduleByDay[${i}].day: invalid day "${entry.day}" ` +
                `— must be one of ${VALID_DAYS.join(', ')}`,
              );
            }
            if (!TIME_RE.test(entry.time)) {
              errors.push(
                `recurrence.scheduleByDay[${i}].time: must be HH:mm (got "${entry.time}")`,
              );
            }
          });
        }
      ...
      if (r.everyNDays !== undefined) {
        ...
        if (!TIME_RE.test(r.everyNDays.time)) {
          errors.push(
            `recurrence.everyNDays.time: must be HH:mm (got "${r.everyNDays.time}")`,
          );
        }
      }
      ...
      if (r.everyNMinutes !== undefined) {
        if (!isPositiveNumber(r.everyNMinutes.n)) {
          errors.push('recurrence.everyNMinutes.n: must be a positive number');
  ...
  if (hasOldRecurrence) {
    if (reminder.recurringSchedule !== undefined) {
      if (!Array.isArray(reminder.recurringSchedule)) {
        ...
        reminder.recurringSchedule.forEach((entry, i) => {
          ...
          if (!TIME_RE.test(entry.time)) {
            errors.push(
              `recurringSchedule[${i}].time: must be HH:mm (got "${entry.time}")`,
            );
          }
        });
      }
    ...
    if (reminder.recurringIntervalMinutes !== undefined) {
      ...
      if (reminder.unacknowledgedRetryMinutes != null) {
        warnings.push(
          'recurringIntervalMinutes and unacknowledgedRetryMinutes are both set ' +
          '— this is usually redundant (see NOTIFICATIONS_SCHEMA.md)',
        );
      }
    }
  ...
  if (reminder.retry !== null && reminder.retry !== undefined) {
    if (!isPositiveNumber(reminder.retry.intervalMinutes)) {
      ...
      warnings.push(
        'retry.intervalMinutes is below the 10-min scheduler tick — will be rounded up',
      );
    }
    if (
      ...
      errors.push(
        `retry.retryUntil: must be HH:mm or null (got "${reminder.retry.retryUntil}")`,
      );
    }
  }
  ...
  if (
    ...
    if (!isPositiveNumber(reminder.unacknowledgedRetryMinutes)) {
      ...
      warnings.push(
        'unacknowledgedRetryMinutes is below the 10-min scheduler tick — will be rounded up',
      );
    }
  }
  ...
  if (
    ...
    warnings.push(
      `notification.scheduledTime (${reminder.notification.scheduledTime}) is out of sync ` +
      `with scheduledTime (${reminder.scheduledTime})`,
    );
  }
  ...
  const hasLinkedTitle =
    typeof reminder.linkedTitle === 'string' && reminder.linkedTitle.trim().length > 0;
